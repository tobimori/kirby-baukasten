...
<body>
  ...
  <script>
    console.log('%ccoralic/kirby-baukasten<?= file_exists($kirby->roots()->base() . '/.git/refs/heads/main') ? '#' . substr(file_get_contents($kirby->roots()->base() . '/.git/refs/heads/main'), 0, 7) : '' ?>', 'font-size: 12px; font-weight: bold; color: #F9E7FF; background-color: #7D3BA8; padding: 10px 16px; margin: 20px 0; border-radius: 4px;')
